...
class CoinMarketCap
  ...
  def self.fetch(symbols)
    url = "https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=#{symbols.join(",")}&convert=EUR&CMC_PRO_API_KEY=#{ENV["COINMARKETCAP_API_KEY"]}"
    uri = URI(url)
    response = Net::HTTP.get(uri)
    json = JSON.parse(response)
    symbols.reduce({}) do |memo, symbol|
      data = json.dig("data", symbol, "quote", "EUR")
